    ...
    <main className={styles.page}>
      <div className={styles.hero}>
        <div className={styles.heroInner}>
          <div>
            <Link className={styles.backLink} href={`/applications/${id}`}>
              <ArrowLeft size={14} /> Back to application
            </Link>
            <h1>Edit application</h1>
            <p>Update the business permit application details for {form.businessName || "this business"}.</p>
          </div>
          <div className={styles.headerMeta}>
            <FilePlus2 size={18} />
            <span>
              Editing
              <strong>BP-2025-{String(id).padStart(4, "0")}</strong>
            </span>
          </div>
        </div>
      </div>
        ...
        <section className={styles.formBody}>
          ...
          {step === 0 && (
            <div className={styles.formGrid}>
              <SelectField label="Application type" required placeholder="Select type" options={APPLICATION_TYPES} value={form.applicationType} onChange={(v) => set("applicationType", v)} />
              <SelectField label="Business type" required placeholder="Select business type" options={BUSINESS_TYPES} value={form.businessType} onChange={(v) => set("businessType", v)} />
              <Field label="Business name" required error={errors.businessName} wide>
                <input value={form.businessName} onChange={(e) => set("businessName", e.target.value)} placeholder="Registered business name" />
              </Field>
              <Field label="Trade name / Franchise" hint="Leave blank if same as business name">
                <input value={form.tradeName} onChange={(e) => set("tradeName", e.target.value)} placeholder="e.g. 7-Eleven, Jollibee" />
              </Field>
              ...
              <Field label="TIN">
                <input value={form.tinNumber} onChange={(e) => set("tinNumber", e.target.value)} placeholder="Tax Identification Number" />
              </Field>
              <Field label="Business address" required error={errors.businessAddress} wide>
                <input value={form.businessAddress} onChange={(e) => set("businessAddress", e.target.value)} placeholder="Street, building, lot/block number" />
              </Field>
              <SelectField label="Barangay" required placeholder="Select barangay" options={BARANGAYS} value={form.barangay} onChange={(v) => set("barangay", v)} />
            </div>
          )}
          ...
          {step === 1 && (
            <div className={styles.formGrid}>
              <Field label="Last name" required error={errors.ownerLastName}>
                <input value={form.ownerLastName} onChange={(e) => set("ownerLastName", e.target.value)} placeholder="Family name" />
              </Field>
              <Field label="First name" required error={errors.ownerFirstName}>
                <input value={form.ownerFirstName} onChange={(e) => set("ownerFirstName", e.target.value)} placeholder="Given name" />
              </Field>
              <Field label="Middle name">
                <input value={form.ownerMiddleName} onChange={(e) => set("ownerMiddleName", e.target.value)} placeholder="Middle name" />
              </Field>
              <Field label="Contact number" required error={errors.ownerContact}>
                <input value={form.ownerContact} onChange={(e) => set("ownerContact", e.target.value)} placeholder="09XX-XXX-XXXX" />
              </Field>
              ...
              <Field label="Home address" wide>
                <input value={form.ownerAddress} onChange={(e) => set("ownerAddress", e.target.value)} placeholder="Complete residential address" />
              </Field>
            </div>
          )}
          ...
          {step === 2 && (
            <div className={styles.formGrid}>
              <SelectField label="Line of business" required placeholder="Select category" options={LINE_OF_BUSINESS} value={form.lineOfBusiness} onChange={(v) => set("lineOfBusiness", v)} wide />
              <Field label="Business area (sq.m.)">
                <input type="number" min="0" value={form.businessArea} onChange={(e) => set("businessArea", e.target.value)} placeholder="Floor area in square meters" />
              </Field>
              <Field label="Number of employees">
                <input type="number" min="0" value={form.numberOfEmployees} onChange={(e) => set("numberOfEmployees", e.target.value)} placeholder="Total employees" />
              </Field>
              <Field label="Capital investment" hint="For new applications">
                <div className={styles.currencyInput}>
                  <span>₱</span>
                  <input type="number" min="0" step="1000" value={form.capitalInvestment} onChange={(e) => set("capitalInvestment", e.target.value)} placeholder="0.00" />
                </div>
              </Field>
              <Field label="Gross sales / receipts" hint="For renewal applications">
                <div className={styles.currencyInput}>
                  <span>₱</span>
                  <input type="number" min="0" step="1000" value={form.grossSales} onChange={(e) => set("grossSales", e.target.value)} placeholder="0.00" />
                </div>
              </Field>
              <fieldset className={`${styles.choiceField} ${styles.fieldWide}`}>
                <legend>Tax incentives <i>Select if applicable</i></legend>
                <div className={styles.choiceGrid}>
                  {TAX_INCENTIVES.map((item) => {
                    const selected = form.incentives.includes(item);
                    return (
                      <button type="button" key={item} className={selected ? styles.choiceSelected : ""} aria-pressed={selected} onClick={() => set("incentives", toggleValue(form.incentives, item))}>
                        {item}
                      </button>
                    );
                  })}
                  ...
                  {REQUIRED_DOCS.map((doc) => {
                    ...
                    return (
                      <label key={doc} className={styles.switchCard}>
                        <input type="checkbox" checked={checked} onChange={() => set("documents", toggleValue(form.documents, doc))} />
                        <span>
          ...
          {step === 4 && (
            <div className={styles.reviewLayout}>
              ...
              <div className={styles.reviewSection}>
                <header><CircleDollarSign size={15} /><h3>Business Information</h3></header>
              ...
              <div className={styles.reviewSection}>
                <header><CircleDollarSign size={15} /><h3>Owner Details</h3></header>
                <div className={styles.reviewGrid}>
              ...
              <div className={styles.reviewSection}>
                <header><CircleDollarSign size={15} /><h3>Business Activity</h3></header>
                <div className={styles.reviewGrid}>
              ...
              <div className={styles.reviewSection}>
                <header><ClipboardList size={15} /><h3>Documents Submitted ({form.documents.length} of {REQUIRED_DOCS.length})</h3></header>
                <div className={styles.reviewGrid}>
                  {REQUIRED_DOCS.map((doc) => (
              ...
              <div className={styles.reviewReady}>
                ...
                <div>
                  ...
                  <p>Review the information above. Click &quot;Save Changes&quot; to update this application.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <footer className={styles.formFooter}>
          <div>
            {step > 0 ? (
              ...
              <Link className={styles.secondaryButton} href={`/applications/${id}`}>
                Cancel
              </Link>
            )}
          </div>
